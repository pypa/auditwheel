...
def test_build_repair_numpy(any_manylinux_container, docker_python, io_folder):
    ...
    if op.exists(op.join(WHEEL_CACHE_FOLDER, policy, ORIGINAL_NUMPY_WHEEL)):
        ...
        docker_exec(
            manylinux_ctr, f"pip wheel -w /io --no-binary=:all: numpy=={NUMPY_VERSION}"
        )
    ...
    output = docker_exec(
        docker_python, "python /auditwheel_src/tests/integration/quick_check_numpy.py"
    )
...
def test_build_wheel_with_binary_executable(
    any_manylinux_container, docker_python, io_folder
):
...
def test_build_wheel_with_image_dependencies(
    with_dependency, any_manylinux_container, docker_python, io_folder
):
        ...
        with pytest.raises(CalledProcessError):
            docker_exec(
                ...
                [
                    "bash",
                    "-c",
                    repair_command.format(policy=target_policy, orig_wheel=orig_wheel),
                ],
            )
...
def test_build_wheel_depending_on_library_with_rpath(
    any_manylinux_container, docker_python, io_folder, dtag
):
...
def test_build_repair_multiple_top_level_modules_wheel(
    ...
    filenames = os.listdir(io_folder)
    assert filenames == [f"multiple_top_level-1.0-{PYTHON_ABI}-linux_{PLATFORM}.whl"]
    orig_wheel = filenames[0]
    assert "manylinux" not in orig_wheel
...
def test_build_wheel_compat(
    target_policy, only_plat, any_manylinux_container, docker_python, io_folder
):
